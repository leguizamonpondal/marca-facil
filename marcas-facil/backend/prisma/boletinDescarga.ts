 ...
 *
 * ── Por qué una por una y no un `createMany` ───────────────────────────────
 *
 * `createMany` con `skipDuplicates` sería más rápido, pero no dice **cuáles**
 * entraron y cuáles no. Acá interesa distinguir tres cosas que un contador
 * global confunde:
 *
 *   - **nuevas** — actas que no estaban
 *   - **repetidas** — ya estaban, típicamente porque se reprocesa una fecha
 *   - **fallidas** — el guardado tiró error
 *
 * Una acta fallida es una marca que no se va a vigilar. Tiene que contarse
 * aparte de una repetida, que es inofensiva. Con 4.000 actas semanales el
 * costo de hacerlo prolijo es de segundos, y la diferencia entre "0 alertas
 * porque no había nada" y "0 alertas porque no se guardó nada" es el producto
 * entero.
 *
 * El `acta` es único en la base, así que reprocesar una fecha es seguro: las
 * que ya estaban se cuentan como repetidas y no se tocan.
 */
async function guardarActas(
  ...
  for (const a of actas) {
    try {
      const existente = await prisma.boletinEntrada.findUnique({
        where: { acta: a.acta },
        select: { id: true },
      });
      if (existente) {
      ...
      await prisma.boletinEntrada.create({
        data: {
          fechaBoletin,
          boletinNumero,
          acta: a.acta,
      ...
      fallidas++;
      // Se registra cada una: son marcas que quedan sin vigilar, no ruido.
      logger.error(`[Boletín] No se pudo guardar el acta ${a.acta}: ${err.message}`);
    }
  }

  return { nuevas, repetidas, fallidas };
}
