...
import { buscarMarcasPublicoINPI, buscarPorTitularINPI, buscarPorCuitINPI } from '../../services/inpiService';
import {
  buscarPorTitularWS,
  buscarPorCuitWS,
...
/**
 * Consulta el Web Service oficial del INPI y, si falla, cae al scraping con Playwright.
 *
 * El WS es oficial, estable y no requiere credenciales para estas consultas.
 * El scraper queda como red de seguridad: si el INPI cambia o el servicio no responde,
 * la funcionalidad degrada en vez de romperse.
 */
async function consultarConFallback(
  etiqueta: string,
  viaWS: () => Promise<MarcaINPI[]>,
  viaScraping: () => Promise<MarcaINPI[]>
): Promise<{ marcas: MarcaINPI[]; fuente: 'ws-inpi' | 'scraping' }> {
  try {
    const marcas = await viaWS();
    return { marcas, fuente: 'ws-inpi' };
  } catch (err: any) {
    logger.warn(`[Factibilidad] WS del INPI falló en ${etiqueta} (${err?.message}). Cayendo a scraping.`);
    const marcas = await viaScraping();
    return { marcas, fuente: 'scraping' };
  }
}
...
// ── GET /api/factibilidad/ws-test — Diagnóstico del Web Service INPI (SIN auth) ─
// Verifica contra el WS oficial. No expone credenciales: estas operaciones
// del INPI son públicas. Sirve para confirmar desde el navegador que Railway
// llega al servicio.
...
router.get('/ws-test', async (req: any, res: Response) => {
  ...
  try {
    ...
    const estado = await verificarWS();
    return res.status(estado.ok ? 200 : 503).json({
      operacion: 'verificarWS',
      ...estado,
      ayuda: 'Probá ?denominacion=ADIDAS  |  ?titular=NIKE  |  ?cuit=30500000003',
    });
  } catch (err: any) {
    logger.error(`[WS-Test] ${err?.message}`);
...
router.get('/test', async (req: any, res: Response) => {
  ...
  try {
    ...
    // Capturar todos los links/tabs de navegación
...
// ── GET /api/factibilidad/buscar-cuit — Marcas de un titular por CUIT ────────
router.get('/buscar-cuit', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
...
/**
 * Convierte la similitud máxima detectada en un porcentaje de registrabilidad.
 * Escala invertida: más similitud = menos registrabilidad.
 * Umbral de recomendación: > 50% (según criterio del estudio).
 */
function calcularRegistrabilidad(antecedentes: any[]): number {
  ...
  return Math.round(base / 5) * 5; // redondear a múltiplos de 5%
}
